...
;['error', 'log', 'info', 'warn', 'trace'].forEach(function (name) {
  ...
  console[name] = function () {
    if (console._trace || console.traceOptions.always) {
      ...
      var sn = '';
      if(process.domain && process.domain.session && process.domain.session.SN){
          sn = '[SN:' + process.domain.session.SN + ']';
      }
      var iQQ = "";
      if(process.domain && process.domain.session && process.domain.session.iQQ){
        iQQ = '[iQQ:' + process.domain.session.iQQ + ']';
      }
      var pad = (arguments[0] && !console.traceOptions.right || !isatty ? ' ' : '');
      arguments[0] = console.traceFormat(__stack[1], name) + pad + sn + pad + iQQ + pad + arguments[0];
    }
